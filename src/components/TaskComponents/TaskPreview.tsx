...
export default function TaskPreview({ task }: { task: TaskInterface }) {
  ...
  return (
    <button
      className='task-card block bg-white dark:bg-dark-gray px-16px py-[2.3rem] rounded-sm w-full max-w-[28rem] min-w-[28rem] shadow-sm mb-[2rem] transition-all duration-500 hover:translate-x-[-5px] hover:translate-y-[-5px] hover:shadow-md'
      onClick={() => {
        setModal(<TaskDetails task={task} />)
        setOverlayActive(true)
      }}
    >
      <h4 className='task-title font-bold text-[1.5rem] mb-[0.8rem] dark:text-white transition-colors duration-500'>
        {task.title}
      </h4>
